...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [["/newUpload", "/stopUpload"], ["/countall"]]
...
async def new_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    WAITING_UPLOAD[update.effective_user.id] = True

    await update.message.reply_text(
        "📥 Upload mode ON\n\n"
        "Ab jitne bhi messages bhejoge, sab questions add honge.\n"
        "Jab complete ho jaye, /stopUpload bhejo."
    )
...
async def stop_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    WAITING_UPLOAD[update.effective_user.id] = False
...
async def receive_questions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    update_file(updated_text, sha, "Admin: added questions (bulk upload)")
...
app.add_handler(CommandHandler("stopUpload", stop_upload))
